...
func (room Room) IsValidRoom(m Map, room_id string) error {
	...
	for exit_dir, exit_room := range room.Exits {
		if !is_inside(exits, exit_dir) {
			return errors.New(fmt.Sprintf("Invalid map: '%s' exits doesn't exist", exit_dir))

		} else if !is_inside(room_names, exit_room) {
			return errors.New(fmt.Sprintf("Invalid map: '%s' room doesn't exist", exit_room))
		}
	}
	...
	for _, item := range room.Items {
		if !is_inside(item_names, item) {
			return errors.New(fmt.Sprintf("Invalid map: '%s' item doesn't exist", item))
		}
	}
	...
	for _, npc := range room.Npcs {
		if !is_inside(npc_names, npc) {
			return errors.New(fmt.Sprintf("Invalid map: '%s' npc doesn't exist", npc))
		}
	}
...
func (npc Npc) IsValidNpc() error {
	...
	if !is_inside(roles, npc.Role) {
		return errors.New(fmt.Sprintf("Invalid map: '%s' role isn't valid", npc.Role))

	// Check quest id
	} else if npc.QuestId != "" && !is_inside(quest_names, npc.QuestId) {
		return errors.New(fmt.Sprintf("Invalid map: '%s' quest id doesn't exist", npc.QuestId))
	}
	...
	if npc.Stats.Hp < 0 {
		...
		return errors.New(fmt.Sprintf("Invalid map: '%s' status doesn't exist", npc.Stats.Status))
	}
...
func (quest Quest) IsValidQuest() error {
	if !is_inside(quest_status, quest.Status) {
		return errors.New(fmt.Sprintf("Invalid map: '%s' status doesn't exist", quest.Status))
	}
